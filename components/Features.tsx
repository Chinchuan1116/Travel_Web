...
type FeatureItemPrompt = {
    title:string;
    icon:string;
    description:string;
}

const FeatureItem = ({title, icon, description}:FeatureItemPrompt) =>{
    return(
        ...
        <div className='max-container padding-container relative w-full justify-end flex'>
            ...
            <div className="z-20 flex w-full flex-col lg:w-[70%]">
                <div className="relative">
                    <Image src='camp.svg' alt="svg" width={50} height={50} className="absolute left-[-5px] top-[-28px] w-10 lg:w-[50px]"/>
                    <h2 className="bold-40 lg:bold-64">Our Features</h2>
                </div>
                <ul className="mt-10 grid gap-10 md:grid-cols-2 lg:mg-20 lg:gap-20">
                    {FEATURES.map((feature)=>(
                        <FeatureItem title={feature.title} key={feature.title} icon={feature.icon} description={feature.description}/>
                    ))}
                </ul>
            </div>
        </div>
